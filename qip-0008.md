...
The stating model will be described by the following algorithm

EOA: `deposit(node) = log10(existing_leaves)`
Contract code: `deposit(code) = log10(existing_leaves) * code_size) / quai_per_byte`
Contract storage: `deposit(storage) = ((integral of log10(expected_slots)) * slot_size (32 bytes)) / quai_per_byte`

As a starting point, 1 QUAI is equal to 1000 bytes. A EOAs and contracts must maintain a balance of log10(existing_leaves). The cost of having to stake for storage should alleviate growth of the tree since developers are incentivized to manage state.

Examples:
The USDC contract itself is 4372 bytes in size and deployed when the trie has 8,000,000 leaves. 
...
When calling storage on data, a transaction will fail if it is writing more data than the staked rent the contract has paid for.

### Modifications to EOAs
EOAs/contracts that attempt to transfer balance resulting in less than 1 QUAI will invoke the `SELFDESTRUCT` opcode on the trie.

### Reclaiming State
After removing state from the contract, the contract owner will be able to unstake the equivalent amount of QUAI from the stating deposit.
